...
- Contribution policy satisfied: CLA, signed commits, required labels, repo accepting PRs. Also check the repo root for a dedicated AI policy file (e.g. `AI_POLICY.md`): a policy that bans agent-created PRs or mandates AI disclosure is an account-integrity hard gate (beetbox/beets, 14 Sep 2026) - stop and record a no-go rather than hiding AI usage; the only compliant path is the human opening the PR personally with the required disclosure.
