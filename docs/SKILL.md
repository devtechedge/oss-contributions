...
4b. Renders the master resume DOCX `docs/Devayan_Mandal.docx` from resume.txt via `scripts/render-resume-docx.py`. That file is a generated artifact with three fixed rules: exactly two pages, 0.4 inch margins, nothing below 10pt. To hold two pages the renderer re-summarises on every run: the open-source section absorbs the squeeze first (full bullets, then condensed one-liners, then a subset plus a `+N more` roll-up), and only if that is not enough are the hand-written sections condensed, cheapest first. It measures text with baked-in Calibri metrics, inflates the result by 12% (`RENDER_SAFETY`, because viewers that substitute a font for Calibri or apply their own line spacing spill onto a third page otherwise), and targets 95% of the budget, because 0.98 fits and 1.00 spills to three pages. **Edit `docs/Devayan_Mandal-resume.txt`, never the DOCX** — the DOCX is rewritten from it on every sync, and it writes only when the bytes change
