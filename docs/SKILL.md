...
- **Dormancy rule (decided 2026-09-16, ping window 14 to 7 days 2026-09-20 per Dev):** "activity" means a human other than us, or a code-review
  bot (CodeRabbit, Greptile, Copilot, Qodo, Devin, cubic-dev-ai). Our own comments do **not** reset
  the clock, so a PR we keep pinging is still dormant. When an open PR has gone **7 days** with no
...
- **Reminder / nudge comment craft (decided 2026-09-22 per Dev):** dormancy pings are not one-liners. Each reminder must be gentle, respectful, kind, and not pushy; at least **five sentences**; and concrete about what the PR already did (files, root cause, tests, linked issue). Use **GitHub-compliant markdown**: blank line between paragraphs, inline `` `code` `` for identifiers/APIs/commands, and `#N` / `Fixes #N` issue refs where relevant. Keep the shared voice (hyphen `-`, never an em dash). Show the full draft through the comment approval gate and post only the approved text verbatim via `--body-file`. When posting a batch, keep a **~1 minute gap** between comments so GitHub does not treat the burst as automation (hardening of the spacing note above; 22 Sep 2026: five approved reminders posted one minute apart with no warnings). After each successful post, fetch the comment back and confirm the body matches; log `last_ping` on the PR row in `docs/triage/triage.json` the same turn. Close clock after a ping remains **7 days** with no real activity.
...
4. Regenerates README in this repo plus professional docs in `devtechedge/jobsearch-private` repo root via `--private-root private`: `Devayan_Mandal-resume.txt`, `linkedin-all-details.txt`, `wellfound.txt`, `linkedin-experience-paste.txt`, and `devayan-all-details.txt` (counts + date only, 22 Sep 2026; Desktop copy is the editing truth, repo-name selections and prose stay Dev's) (moved from `docs/generated/` 21 Sep 2026; `docs/generated/` no longer exists in this repo. The profile README sync renders its fragment in-memory.)
