...
When an upstream PR merges, do not independently edit README, resume, LinkedIn source, profile README, Wellfound source, or the repository About text. Trigger the ledger workflow instead. The master resume DOCX is exempt: it is hand-maintained and decoupled from resume.txt (19 Sep 2026), so edit it directly or via `scripts/patch-resume-docx.py`.
...
4. Regenerates README, resume.txt, linkedin-all-details.txt, `docs/wellfound.txt`, and `docs/generated/linkedin-experience-paste.txt` (the only file left in `docs/generated/` since 18 Sep 2026: the profile mirrors and resume artifacts were deleted and their publishers unwired. The profile README sync renders its fragment in-memory.)
4b. Leaves the master resume DOCX `docs/Devayan_Mandal.docx` alone (decoupled 19 Sep 2026): the sync never renders or overwrites it, and `validate()` does not check it, so Dev can audit and edit it in Word at any time with no connection to resume.txt. The DOCX still moves on every merge and professional update, by explicit agent action rather than by sync: the agent session handling the change runs `scripts/patch-resume-docx.py` (`--check` to reconcile against the merged set, `--add`/`--remove` for bullets), which inserts bullets in `IMPORTANCE`/`REPO_TIER` significance order with PR hyperlinks while every other byte passes through untouched. `scripts/render-resume-docx.py` stays as the ranking/condensing reference library (and full-render history) but is no longer executed by the sync. Rule for future targets: same decoupling applies to any new hand-maintained binary, never wire it into the sync render path.
