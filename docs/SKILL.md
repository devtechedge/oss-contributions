...
4. Regenerates README in this repo plus professional docs in `devtechedge/jobsearch-private` repo root via `--private-root private`: `Devayan_Mandal-resume.txt`, `linkedin-all-details.txt`, `wellfound.txt`, and `linkedin-experience-paste.txt` (moved from `docs/generated/` 21 Sep 2026; `docs/generated/` no longer exists in this repo. The profile README sync renders its fragment in-memory.)
