...
- **Description:** Public ledger of upstream open-source contributions: 26 merged pull requests across TypeScript, Rust and Python. Merged into Anza Kit, Better Auth, Biome, cargo-about, js-stellar-sdk, maturin, node-postgres, pnpm, pytest-env, Recharts, reviewgate, Rspress, ruff, SQLMesh, stellar-docs, thirdweb JS, and tracelens.
...
| Repo | PR | What | Merged |
| --- | --- | --- | --- |
| <img src="https://github.com/leo-aa88.png?size=40" width="18" /> [leo-aa88/reviewgate](https://github.com/leo-aa88/reviewgate) | [#172](https://github.com/leo-aa88/reviewgate/pull/172) | Corrects DESIGN.md section 10.14 Hunk entry state to match the shipped code-token rule from PR 144: each establishing context line must scan clean and carry a code token, with blank lines neutral, so docstring prose can no longer read as a normal code position. | 19 Sep 2026 |
...
## 3. skills-hub
- **URL:** https://github.com/devtechedge/skills-hub
- **Language:** JavaScript
- **Topics:** None
...
Private master location for Dev's agent skills. Every platform (opencode, grokbot, others) can push/pull here instead of copying from different physical locations.

Renamed from `github-repo-presentation` on 2026-09-20. Old URL redirects: `github.com/devtechedge/github-repo-presentation` -> `github.com/devtechedge/skills-hub`.

Keep this repository private. Never copy zips into public product repos.

## Layout

```
skills-hub/
  github-repo-presentation/   # Rev 12 as of 2026-09-18 (local wins over old Rev 11)
    SKILL.md
    HANDOFF.md
    readme-skeleton.md
    mit-license.txt
    references/
    github-repo-presentation-revision-12.zip
  oss/                        # mirror of oss-contributions docs/SKILL.md + PATTERNS.md
    SKILL.md
    PATTERNS.md
    sync-from-github.py
    sync-from-github.sh
  self-learn/                 # opencode self-learning loop rules
  dev-memory/                 # memory protocol (no memory data, no PII)
  applypilot/
  job-application-agent/      # templates only, no real candidate PII
  code-review/
  ... (one folder per skill in ~/.agents/skills)
  github-repo-presentation-revision-11.zip   # legacy root zip, kept for history
```

## Freshness

- `github-repo-presentation`: **Rev 12 (2026-09-18)** is current. Rev 11 (2026-09-01) files + root zip are history.
- `oss`: canonical source stays `devtechedge/oss-contributions` (`docs/SKILL.md` + `docs/PATTERNS.md`). This hub is a mirror.
- After every skill update: push here, then pull from grokbot (or GitHub Desktop fetch then push into grokbot).

## Download (Grok pattern)

- Click a `*-revision-*.zip` to download, or clone with GitHub Desktop into `Documents\GitHub\skills-hub`.
- Re-upload the zip at grok.com/skills-and-connectors after a Rev bump.


---

## 4. proliquid-twin
- **URL:** https://github.com/devtechedge/proliquid-twin
- **Language:** TypeScript
- **Topics:** apexcharts, astro, binance, hyperliquid, react, tailwindcss, trading-terminal, typescript, vite, zustand
- **Description:** proliquid_ twin: all-in-one trading terminal with an Astro landing page plus a React terminal running live Binance and Hyperliquid prices, real kline candlesticks, multi-timeframe charts, order ticket, orderbook and news feed. Vite, Zustand, ApexCharts, Tailwind. Dark/light themes. Client-side demo: execution simulated, Supabase not provisioned.

### README.md

# proliquid_ — all-in-one trading terminal

One workspace for every exchange: live prices, real candlesticks, order ticket, orderbook, and news across crypto, TradFi, commodities, FX, and pre-IPO markets.

[![Live Demo](https://img.shields.io/badge/Live%20Demo-Vercel-black?logo=vercel)](https://proliquid-terminal.vercel.app)
[![CI](https://github.com/devtechedge/proliquid-twin/actions/workflows/ci.yml/badge.svg)](https://github.com/devtechedge/proliquid-twin/actions/workflows/ci.yml)
[![Astro](https://img.shields.io/badge/Astro-5-black?logo=astro)](https://astro.build/)
...
- **Terminal:** https://proliquid-terminal.vercel.app
- **Landing page:** https://proliquid-marketing.vercel.app

> **Status:** This is a client-side deploy. Prices and candles are live public data (Binance, Hyperliquid, Frankfurter — no keys needed). Order execution is simulated locally, the news feed is sample data, and there is no login yet. Supabase is schema-only and not provisioned.

---

## Screenshots

| Trade terminal (dark) | Trade terminal (light) |
|---|---|
| ![Trade terminal](docs/screenshots/01-terminal-trade.png) | ![Light theme](docs/screenshots/02-terminal-light.png) |

| Landing hero | Landing markets |
|---|---|
| ![Landing](docs/screenshots/03-landing-hero.png) | ![Markets](docs/screenshots/04-landing-markets.png) |

---

## Features

- Live price engine: Binance WebSocket ticks with Kraken/Coinbase fallback, Hyperliquid mids, ECB FX reference rates
- Real candlesticks: Binance kline history + live candle stream, Hyperliquid candles, daily FX series
- Order ticket with long/short, limit/market, leverage, and a working local order blotter
- Depth-style orderbook, market list with live/stale feed badges, multi-timeframe chart
- Dark / light themes with hover-reveal overlay scrollbars across both apps
- Marketing landing with market explorer, exchange grid, news preview, pricing, and FAQ
...
| Layer | Technology |
|-------|------------|
| Landing | Astro 5 (static), vanilla JS |
| Terminal | React 18, Vite 6, Zustand, ApexCharts, Tailwind CSS v3 |
| Data | Public exchange APIs only (no backend yet) |
| Key storage | AES-256-GCM plan, device-local (IndexedDB) |
| Hosting | Vercel (two projects, git-linked) |
...
Copy `.env.example` to `.env` when wiring Supabase / OpenCode keys. Nothing in this repo requires keys to run.

### Tests

```bash
# unit (kline merge/nudge logic)
pnpm --filter @proliquid/terminal test

# browser smokes (uses local Chrome via PW_CHANNEL; CI uses the Playwright bundle)
PW_CHANNEL=chrome pnpm --filter @proliquid/terminal exec playwright test
```
...
```
apps/terminal    # React trading terminal (Vite)
apps/marketing   # Astro landing page (static)
packages/*       # shared types, utils, UI
infra/supabase   # Postgres migrations + edge functions (not yet provisioned)
docs/screenshots # README captures + social preview
```
...
## 5. remix-render-to-string-navigation
...
## 10. certifications
- **URL:** https://github.com/devtechedge/certifications
- **Language:** Not specified
- **Topics:** anthropic, aws, azure, certifications, langgraph, openai, blockchain, coursera, data-science, deep-learning, professional-development, tensorflow
- **Description:** Verified certification archive for Dev M with 30 entries across Anthropic MCP, OpenAI agents, LangGraph deep and ambient agents, AWS, Azure AZ-900, TensorFlow, deep learning, JHU data science, IBM data science, blockchain, VR, self-driving cars, Google IT automation, six sigma and Forage, each with issuer, date, verify link and local PDF.

### README.md

# Certifications

Public archive of verified professional certifications held by Dev M, with issuer, date, verification link, and local PDF where available.

[![License](https://img.shields.io/badge/License-MIT-green)](LICENSE)
[![Certifications](https://img.shields.io/badge/Certifications-30-blue)](README.md)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-dev--ma-0A66C2?logo=linkedin)](https://www.linkedin.com/in/dev-ma/)

---

## What is here

- 30 entries grouped by area, newest first inside each group.
- 22 entries have a local PDF under `certs/`.
- 8 entries are verify-link only (no PDF in hand, listed with issuer and date).
- Source for names, issuers, dates, and verification URLs: LinkedIn Licenses and Certifications section, as of September 2026.

> Status: docs-only repo. There is no app to deploy, so there is no demo server. Browse the tables below, open a PDF in `certs/`, or follow a Verify link to the issuer.

---

## AI agents and applied AI

| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| Model Context Protocol: Advanced Topics | Anthropic | Sep 2026 | [Verify](https://verify.skilljar.com/c/g9szj3w428ix) | [PDF](certs/anthropic-model-context-protocol-advanced-topics.pdf) |
| AI Fluency for Builders | Anthropic with CodePath | Sep 2026 | [Verify](https://verify.skilljar.com/c/bxe2dhq82w6c) | [PDF](certs/codepath-anthropic-ai-fluency-for-builders.pdf) |
| Agents and Workflows | OpenAI Academy | Sep 2026 | [Verify](https://academy.openai.com/public/certificate/cv4gh56iyc) | [PDF](certs/openai-academy-agents-and-workflows.pdf) |
| Deep Agents with LangGraph | LangChain Academy | Jun 2026 | [Course](https://academy.langchain.com/courses/deep-agents-with-langgraph) | [PDF](certs/deep-agents.pdf) |
| Ambient Agents with LangGraph | LangChain Academy | Jun 2026 | [Course](https://academy.langchain.com/courses/ambient-agents) | [PDF](certs/ambient-agents-with-langgraph.pdf) |

What each one covers:

- MCP Advanced Topics: JSON-RPC messaging, Stdio handshakes, StreamableHTTP and SSE, sessions, client-side sampling, telemetry, roots-based security, stateful vs stateless transport design.
- AI Fluency for Builders: 4D framework of Delegation, Description, Discernment, and Diligence, plus context design, acceptance criteria, evaluation, and shipping discipline.
- Agents and Workflows: directing, reviewing, and optimizing agents for structured tasks with strong context, target outputs, boundaries, human review, and reusable skills.
- Deep Agents: stateful execution, context management, sub-agent delegation, multi-agent orchestration, evaluation with LangSmith.
- Ambient Agents: background agents, state management, email automation, evaluation with LangGraph and LangSmith.
...
| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| AWS Educate Introduction to Generative AI | Amazon Web Services | Aug 2025 | [Verify](https://www.credly.com/badges/24356848-8540-4eac-bcc3-40db36303d7c/public_url) | Link only |
| AWS Fundamentals Specialization | Amazon Web Services | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/8W8Q9UJSFR7R) | [PDF](certs/coursera-aws-fundamentals.pdf) |
| Microsoft Certified: Azure Fundamentals (AZ-900) | Microsoft | Aug 2020 | [Verify](https://learn.microsoft.com/api/credentials/share/en-us/DevayanMandal-5513/3841D21B0E5E588F?sharingId) | [PDF](certs/microsoft-azure-fundamentals.pdf) |
...
| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| DeepLearning.AI TensorFlow Developer Professional Certificate | DeepLearning.AI | Aug 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/MB53RUAWEP96) | [PDF](certs/coursera-deeplearning-ai-tensorflow-developer.pdf) |
| Deep Learning Specialization | DeepLearning.AI | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/X6UWWBABR5XA) | [PDF](certs/coursera-deeplearning-ai-deep-learning.pdf) |
| IBM AI Engineering Professional Certificate | IBM | Jul 2020 | [Verify](https://www.credly.com/badges/8b611162-933c-4e19-8ce9-91bd7170c332/public_url) | Link only |
| IBM Applied AI Professional Certificate | IBM | Jul 2020 | [Verify](https://www.credly.com/badges/da1a5658-50d9-4f06-99ab-e273c94b0e10/public_url) | Link only |
| Artificial Intelligence Foundations Specialization | Coursera | Jul 2020 | [Verify](https://www.credly.com/badges/80e49cd0-715c-40b8-bdbb-d25aebfd2ad4/public_url) | Link only |
| Applied Data Science Specialization | Coursera | Jul 2020 | [Verify](https://www.credly.com/badges/ba78b3be-4ac2-4f3a-bae4-1278f908df0b/public_url) | Link only |
...
| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| Data Science Specialization | The Johns Hopkins University | Aug 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/9WXW969VYD5S) | [PDF](certs/coursera-jhu-data-science.pdf) |
| Data Science: Statistics and Machine Learning | The Johns Hopkins University | Aug 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/WUTRNV3D9VN3) | [PDF](certs/coursera-jhu-data-science-statistics-and-machine-learning.pdf) |
| Data Science: Foundations using R | The Johns Hopkins University | Aug 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/XZJJ2823YZJJ) | [PDF](certs/coursera-jhu-data-science-foundations-using-r.pdf) |
| IBM Data Science Professional Certificate | IBM | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/DRBARHHVEX6F) | [PDF](certs/coursera-ibm-data-science.pdf) |
| Introduction to Data Science | IBM | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/DN47EG8AZW2L) | [PDF](certs/coursera-ibm-introduction-to-data-science.pdf) |
...
| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| Google IT Automation with Python Professional Certificate | Google | Sep 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/JAAMZGPCF42X) | [PDF](certs/coursera-google-it-automation-with-python.pdf) |
| Google IT Support Professional Certificate | Google | Aug 2020 | [Verify](https://www.credly.com/badges/0b7b6687-2332-4691-962c-215b61e1d1f2/public_url) | [PDF](certs/coursera-google-it-support.pdf) |
| Six Sigma Green Belt | Kennesaw State University | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/VDGA9TV9GG3V) | [PDF](certs/coursera-kennesaw-six-sigma-green-belt.pdf) |
...
| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| Self-Driving Cars Specialization | University of Toronto | Aug 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/LHYPGGJVV75V) | [PDF](certs/coursera-utoronto-self-driving-cars.pdf) |
| Blockchain Specialization | University of California, Irvine | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/HT9CWLVYLDH9) | [PDF](certs/coursera-uci-blockchain.pdf) |
| Virtual Reality Specialization | University of London | Jul 2020 | [Verify](https://www.coursera.org/account/accomplishments/specialization/4XR3YMZUXHE4) | [PDF](certs/coursera-uol-virtual-reality.pdf) |
...
| Certification | Issuer | Date | Verify | Local file |
|---|---|---|---|---|
| Deloitte Australia Technology Consulting Job Simulation | Forage | Jun 2020 | [Verify](https://www.theforage.com/simulations/deloitte-au/technology-consulting-virtual-internship-5972) | [PNG](certs/deloitte-forage.png) |
| J.P. Morgan Software Engineering Job Simulation | Forage | Jun 2020 | [Verify](https://www.theforage.com/simulations/jpmorgan/software-engineering-btjx) | [PNG](certs/jp-morgan-forage.png) |

---

## Reference only, not on LinkedIn

These three AWS Academy courses are kept for personal reference. They are not listed in the LinkedIn Certifications section.

| Course | Issuer | Date | Verify |
|---|---|---|---|
| AWS Academy Graduate - Machine Learning Foundations | Amazon Web Services | May 2023 | [Verify](https://www.credly.com/badges/ce8515b7-dbbb-49ce-a8d8-41aca134e99c/public_url) |
| AWS Academy Graduate - Cloud Architecting | Amazon Web Services | Feb 2023 | [Verify](https://www.credly.com/badges/bc48d311-90c4-46e2-ac5e-eb60809a63de/public_url) |
| AWS Academy Graduate - Cloud Foundations | Amazon Web Services | Jan 2023 | [Verify](https://www.credly.com/badges/c535af86-9d85-4edb-9acd-dc38eb267e8c/public_url) |
...
```bash
git clone https://github.com/devtechedge/certifications.git
cd certifications
ls certs
```

Open any PDF in `certs/` with your local reader. Use the Verify links above to confirm each credential with the issuer.
...
- **Description:** AetherFlow is a local-first visual flowchart IDE. Drag nodes on a pan/zoom canvas, compile the graph, and run a step simulator with time-travel snapshots. Gemini plus Gmail, Drive, and Docs nodes. Public Vercel keeps graphs in localStorage. Google sign-in links Workspace APIs; unsigned runs use mock payloads. React, Vite, TypeScript, Tailwind. MIT.
...
- **Language:** Python
- **Topics:** fastapi, langchain, langgraph, python, serverless, vercel, ai-agents, langserve, langsmith, multi-agent, pgvector, rag, autonomous-agents, hitl, human-in-the-loop, sre, sse, streaming, dashboard
- **Description:** AEGIS is a multi-agent operations cortex with a live dashboard. Supervisor plus specialist agents (SRE, knowledge, coder, evaluator, communicator), Hybrid RAG, HITL gates, streaming SSE, and a live LangGraph visualizer. FastAPI, LangChain, LangGraph, LangSmith. Open /ui, toggle Demo vs Live. Demo is instant simulation; Live needs API keys. MIT.
...
## 16. devtechedge.github.io
- **URL:** https://github.com/devtechedge/devtechedge.github.io
- **Language:** HTML
- **Topics:** None
- **Description:** Personal site and privacy policy
...
## 31. healthcare-deep-memory-agents
...
- **Description:** Jobrow indexes still-open US tech roles from public Greenhouse, Ashby, and Lever board APIs. 50 companies, 5,000+ open roles. Table-first search, same-day close when a board drops a posting. TanStack Start, TypeScript, Neon Postgres. Independent index with instant role fetches via API
...
## 40. pygbif
- **URL:** https://github.com/devtechedge/pygbif
- **Language:** Python
- **Topics:** None
- **Description:** GBIF Python client
...
## 49. signalpost-citation-validator
...
## 77. metamask-sdk-empty-placeholder
